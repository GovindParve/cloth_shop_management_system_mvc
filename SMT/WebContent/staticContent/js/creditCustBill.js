...
function regCreditCustomerBill123()
{
	...
	var cashCard_cashAmount = 0;
	var cashCard_cardAmount = 0;
	...
	{
		cashCard_cashAmount = $('#cashupi_cashAmount1').val();
		cashCard_cardAmount = $('#cashAndCard_upiAmount').val();
		
		if(cashCard_cashAmount == undefined || cashCard_cashAmount == null || cashCard_cashAmount == "" || cashCard_cashAmount == " ")
		{
			myAlert("Please Enter Cash Amount111");
			document.custord.btnSubmit.disabled = false;
			return false;
		}
		else
		...
		{
			myAlert("Cash Amount + Card Amount Must be Equals to Net Paid Amount 111");
			document.custord.btnSubmit.disabled = false;
			return false;
		}
		else{}
	...
	if(paymentMode == "cashAndupi")
	{
		...
		else
		{
			...
			if(cashupi_cashAmount.match(checkCashAmt))
			{
				...
				myAlert("Please Enter Valid Cash Amount 22");
				document.custord.btnSubmit.disabled = false;
				return false;
			}
		}
		...
		{
			myAlert("Please Enter Upi Cash Amount 33");
			document.custord.btnSubmit.disabled = false;
			return false;
		}
		else
...
//Old getitemData1() Not in Use
function getitemData1Old()
{
	var rowDelete = 0;
		...
		$.each(jsonData,function(i,v)
		{
			...
			$("#list4").jqGrid({
				...
				colNames:['pk_temp_id', 'item_id', 'Barcode<br>No', 'Category','Category<br>Id','Sub<br>Category', 'Sub<br>Category<br>ID', 'Product', 'Product<br>Id', 
						  'Style','HSN/<br>SAC', 'Roll<br>Size', 'Available<br>Quantity', 'Quantity', 'Size', 'Good Receive Quantity', 'S.P.<br>/Unit', 'fixedSalePrice', 'S.P.<br>W/O<br>Tax', 'Dis<br>%', 'Discount<br>Amount', 
						  'S.P.<br>After<br>Discount', 'GST<br>%', 'Tax<br>Amount', 'Tax Amount<br>After<br>Discount', 'IGST%', 'Total<br>Amount', 'Employee<br>Name', 'forTotal', 'fkSuppId', 'fkShopId','BillNo'],
				colModel:[ 
				          {
				        	  name:'pkTempid',
				        	  hidden:true,
...
function getitemData1()
{
	...
	if(Tempbillid == null || Tempbillid == "" || Tempbillid == " " || Tempbillid == undefined)
	{
		if(value == null || value == "" || value == " " || value == undefined)
		{
			var input = document.getElementById('itemName'), list = document.getElementById('itemId_drop'), i, barcodeNo, catName, itemName, hsnsacno, productId, size;
			for (i = 0; i < list.options.length; ++i)
			{
				if (list.options[i].value === input.value)
				{
	...
	$.post('/SMT/jsp/utility/controller.jsp',params,function(data)
	{
		...
		$.each(jsonData,function(i,v)
		{
			...
			$("#list4").jqGrid({
				datatype: "local",
				colNames:['pk_temp_id', 'item_id', 'Barcode<br>No', 'Category','Category<br>Id','Sub<br>Category', 'Sub<br>Category<br>ID', 'Product', 'Product<br>Id',
						 'Style','HSN/<br>SAC', 'Roll<br>Size', 'Available<br>Quantity', 'Quantity', 'Size', 'Good Receive Quantity', 'S.P.<br>/Unit', 'fixedSalePrice', 'S.P.<br>W/O<br>Tax', 'Discount<br>%', 'Discount<br>Amount',
						 'S.P.<br>After<br>Discount', 'GST<br>%', 'IGST<br>%', 'Tax<br>Amount', 'Tax Amount<br>After Discount', 'Total<br>Amount', 'Employee<br>Name', 'forTotal', 'fkSuppId', 'fkShopId','BillNo'],
				colModel:[
							{
								name:'pkTempid',
								hidden:true,
