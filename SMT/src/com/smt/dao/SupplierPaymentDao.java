...
public class SupplierPaymentDao
{
	...
	public List<SupplierPaymentDetail> getSupplierUnpaidPaymentDetailsAsBill(String fkSupplierId) {
		...
		try {
			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();
			Query query = session.createSQLQuery("select supplier_name, BillNo, pending_bill_payment FROM supplier_details RIGHT JOIN goodreceive ON supplier_details.supplier_id = goodreceive.FksuppId WHERE goodreceive.FksuppId=:fkSupplierId ORDER BY goodreceive.PkGoodRecId DESC LIMIT 1");		
			query.setParameter("fkSupplierId", fkSupplierId);
			List<Object[]> list = query.list();
			supplierList = new ArrayList<SupplierPaymentDetail>(0);
