...
public class GoodReciveDao {
	...
	public void updateBarcodeNoWiseStockEditPurchase(Long barcodeNo, Double quantity, Double buyPrice, Double salePrice, Long shopId)
	{
		System.out.println("Sell EditPurchase barcodeNo :- "+barcodeNo);
		System.out.println("Sell EditPurchase quantity :- "+quantity);		
		HibernateUtility hbu=null;
		Session session=null;
		Transaction transaction=null;
		Long pk=null;
		try{
			...
			Double quantityDefferance = stockOriginalQuantity-stockQuantity;
			Double updatedOriginalQuantity = quantity;
			Double  updatedQuantity = quantity-quantityDefferance;
			...
			gReceipt.setOriginalQuantity(updatedOriginalQuantity);
			gReceipt.setBuyPrice(buyPrice);
			gReceipt.setSalePrice(salePrice);
	...
	public void minusOldQuantityFromGoodReceive(Long item_id, String oldSaleQuantityToUpdateStock, String rollSize, String sQTY, String size1)
	{
		HibernateUtility hbu = null;
		Session session = null;
		Transaction transaction = null;
		
		System.out.println("In Dao  updateQuantity Good Receive item_id :- "+item_id);
		System.out.println("In Dao editSaleInvoceBilling oldSaleQuantityToUpdateStock :- "+oldSaleQuantityToUpdateStock);
		System.out.println("In Dao editSaleInvoceBilling rollSize :- "+rollSize);
		System.out.println("In Dao editSaleInvoceBilling sQTY :- "+sQTY);
		System.out.println("In Dao editSaleInvoceBilling size1 :- "+size1);
		...
		try {
			...
			if (size1.equalsIgnoreCase("meter")	|| size1.equalsIgnoreCase("mtr")) 
			{
				...
				double mQty = Double.parseDouble(oldSaleQuantityToUpdateStock);
				...
				updateSoldQuanty = (Double)(soldQuant - Double.parseDouble(oldSaleQuantityToUpdateStock));
			} else {
				updatequnty = (Double)(quant + Double.parseDouble(oldSaleQuantityToUpdateStock));
				updateSoldQuanty = (Double)(soldQuant - Double.parseDouble(oldSaleQuantityToUpdateStock));
	...
	public void updateQuantity(Long item_id, String quantity, String rollSize, String sQTY, String size1)
	{
		...
		System.out.println("In Dao  updateQuantity Good Receive item_id :- "+item_id);
		System.out.println("In Dao editSaleInvoceBilling quantity :- "+quantity);
		System.out.println("In Dao editSaleInvoceBilling rollSize :- "+rollSize);
		System.out.println("In Dao editSaleInvoceBilling sQTY :- "+sQTY);
		System.out.println("In Dao editSaleInvoceBilling size1 :- "+size1);
		...
		try {
			...
			if (size1.equalsIgnoreCase("meter")	|| size1.equalsIgnoreCase("mtr")) 
			{
				...
				updateSoldQuanty = (Double)(soldQuant + Double.parseDouble(quantity));
				System.out.println("sagar change size in dao Update Good Receive Quantity :- "+updatequnty);
			}
	...
	public List<PreviousGoodReceive> getPreGoodReceive(String billNo) {
		...
		try
		{
			...
			for (Object[] object : list)
			{
				...
				/*String setContactPerson = object[5].toString();
				System.out.println("In Good Receive Dao setContactPerson :- "+ setContactPerson);
				
				String setBarcodeNo = object[4].toString();
				System.out.println("In Good Receive Dao setBarcodeNo :- "+ setBarcodeNo);*/
...
//				stockList.add(reports); 
	...
	//Good Receive Edit Module Code Start From Here
	
	public void removeOddPurchaseQTYFromStockAfterEditPurchaseBillByVoucherNo(Long voucherNo, String fkShopId)
	{
		GoodReciveDao dao=new GoodReciveDao();
		List list12 = dao.getPurchaseBillItemListToUpdateStockQTYForPurchaseBillEdit(voucherNo, fkShopId);
		
		for(int i=0; i<list12.size(); i++)
		{
			GoodReceiveEditBean bean =(GoodReceiveEditBean)list12.get(i);
			
			long pkGoodRecIdDB = bean.getPkGoodRecId();
			Long voucherDB = bean.getVoucherNo();
			Long fkProductIdDB = bean.getFkProductId();
			Double purchaseBillQuantityDB = bean.getQuantity();
			...
			try{
				...
				//Query query = session.createSQLQuery("SELECT COUNT(*) from stock_details where fk_Product_Id="+fkProductIdDB);
				...
				if(intVal==0)
				{
					...
					stock.setQuantity(purchaseBillQuantityDB);
					stock.setItemName(itemNameDB);
					stock.setCatName(categoryNameDB);
					DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
					Date date = new Date();
					stock.setUpdateDate(date);
					stock.setFkCategoryId(fkcategoryIdDB);
					stock.setFkShopId(fkShopIdDB);
					
					session.save(stock);
				} else if(intVal==1) {
					//if product is available in Stock table then update its quantity
					
					//Query query2 = session.createQuery("from Stock where fk_Product_Id="+fkProductIdDB);
					...
					Double  updatedQuantity = quantity2-purchaseBillQuantityDB;
					System.out.println("Update Quantity :- "+updatedQuantity);
					gReceipt.setQuantity(updatedQuantity);
					DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
					Date date = new Date();
					gReceipt.setUpdateDate(date);
					session.update(gReceipt);
				}
				transaction.commit();
			}catch(Exception e){
				System.out.println("Exception In removeOddPurchaseQTYFromStockAfterEditPurchaseBillByvoucherNo");
				e.printStackTrace();
			} finally {
				if(session!=null) {
					hbu.closeSession(session);
			...
			voucherDB = 0l;;
			fkProductIdDB =0l;
			purchaseBillQuantityDB = 0d;
		}
	}
	
	public List getPurchaseBillItemListToUpdateStockQTYForPurchaseBillEdit(Long voucherNo, String fkShopId)
	{
		HibernateUtility hbu=null;
		Session session=null;
		List<GoodReceiveEditBean> shopDetailsList=null;
		try{
			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();

			Query query=session.createSQLQuery("SELECT PkGoodRecId, voucherNo, fkProductId, Quantity, ItemName, CategoryName, fkCatId, fkShopId FROM goodreceive WHERE voucherNo="+voucherNo+" AND fkShopId="+fkShopId);
			List<Object[]> list = query.list();

			shopDetailsList= new ArrayList<GoodReceiveEditBean>(0);

			for (Object[] o : list) {	
				GoodReceiveEditBean bean = new GoodReceiveEditBean();
				
				bean.setPkGoodRecId(Long.parseLong(o[0].toString()));
				bean.setVoucherNo(Long.parseLong(o[1].toString()));
				...
				bean.setCategoryName((o[5].toString()));
	...
	//deletePurchaseBillRecordFromGoodreceiveByBillNo
	public void deletePurchaseBillRecordFromGoodreceiveByVoucherNo(Long voucherNo, String shopIdSession)
	{
		Transaction tx = null;
		HibernateUtility hbu = null;
		Session session = null;
		...
		try {
			...
			Query query = session.createSQLQuery("DELETE from goodreceive WHERE voucherNo = "+voucherNo+" AND fkShopId="+shopIdSession);
			int seletedRecords = query.executeUpdate();
			System.out.println("Number of Records deleted = "+seletedRecords);
			//list = query.list();
			tx.commit();
		} catch (Exception e) {
				e.printStackTrace();
	...
	//deleteBarcodeNumberWiseStockRecordFromBarcodeNumberWiseStockDetailsTableByBarcodeNo
	public void deleteBarcodeNumberWiseStockRecordFromBarcodeNumberWiseStockDetailsTableByBarcodeNo(Long barcodeNo, String shopIdSession)
	{
		Transaction tx = null;
		HibernateUtility hbu = null;
		Session session = null;
		...
		try {
			...
			session = hbu.getHibernateSession();
			tx = session.beginTransaction();
			Query query = session.createSQLQuery("DELETE from barcode_number_wise_stock_details WHERE barcode_no = "+barcodeNo+" AND fk_shop_id="+shopIdSession);
			int seletedRecords = query.executeUpdate();
			System.out.println("Number of Records deleted = "+seletedRecords);
			//list = query.list();
			tx.commit();
