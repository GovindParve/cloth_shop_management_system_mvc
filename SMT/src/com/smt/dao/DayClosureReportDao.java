...
public class DayClosureReportDao
{
	...
	public List getTodayCreditCustCashAmountDao(String userType, String userName)
	{
		...
		try {
			...
			{	
				//query = session.createSQLQuery("select ob.pkOtherBillId, SUM(ob.SalePrice-Discount) from otherbill ob where ob.Date='"+date+"' AND ob.payment_mode='cash'");
				query = session.createSQLQuery("select COUNT(*), COALESCE(sum(ob.cashCard_cashAmount),0) from (select DISTINCT BillNo, cashCard_cashAmount from creditcustomerbill WHERE Date = :date AND (payment_mode='Payment Mode' OR payment_mode='cash') AND Quantity > 0) ob;");
			}
			else
			{
				//query = session.createSQLQuery("select ob.pkOtherBillId, SUM(ob.SalePrice-Discount) from otherbill ob where ob.Date='"+date+"' AND ob.payment_mode='cash' AND ob.EmpId_Fk="+uid);
	...
	//upi payment
	public List getTodayUpiAmountDao(String userType, String userName)
	{
		// TODO Auto-generated method stub
		HibernateUtility hbu = null;
		Session session = null;
		...
		try {
			...
			{	
				//query = session.createSQLQuery("select ob.pkOtherBillId, SUM(ob.SalePrice-Discount) from otherbill ob where ob.Date='"+date+"' AND ob.payment_mode='cash'");
				//query = session.createSQLQuery("select COUNT(*), COALESCE(sum(ob.cashupi_UpiAmount),0) from (select DISTINCT BillNo, cashupi_UpiAmount from creditcustomerbill WHERE Date = :date AND (payment_mode='cashAndupi' OR payment_mode='Upi') AND Quantity > 0) ob;");
				query = session.createSQLQuery("select COUNT(*), COALESCE(sum(ob.cashupi_UpiAmount),0) from (select DISTINCT BillNo, cashupi_UpiAmount from otherbill WHERE Date = :date AND (payment_mode='cashAndupi' OR payment_mode='Upi') AND Quantity > 0) ob;");
			}
			else
			{
				//query = session.createSQLQuery("select ob.pkOtherBillId, SUM(ob.SalePrice-Discount) from otherbill ob where ob.Date='"+date+"' AND ob.payment_mode='cash' AND ob.EmpId_Fk="+uid);
			...
			list = query.list();
			System.out.println("SIZE OF LIST getTodayUpiAmountDao ====> "+list.size());
		} catch (Exception e) {
			e.printStackTrace();
			// TODO: handle exception
		}
