...
public class dayClosureReportHelper
{
	public Map getTodayCashAmounthelper(String userType, String userName)
	{
		...
		public Map getTodayUpithelper(String userType, String userName)
		{
			// TODO Auto-generated method stub
			DayClosureReportDao dao = new DayClosureReportDao();
			List list = dao.getTodayUpiAmountDao(userType, userName);
			System.out.println("today Cash list size ====> "+list.size());
			Map map = new HashMap();
			for (int i = 0; i < list.size(); i++)
			{
				...
				if (o[1] == null)
				{
					bean.setTodayUpiAmt("0");
				} else {
					bean.setTodayUpiAmt(o[1].toString());
					System.out.println("today cash AMOUNT ==========> "+Double.parseDouble(o[1].toString()));
					System.out.println("today cash AMOUNT ==========> "+bean.getTodayUpiAmt());
				}
				map.put(bean.getTodayUpiAmt(), bean);
			}
			return map;
		}
	...
	public Map getTodayUpiCustCashAmounthelper(String userType, String userName)
	{
		// TODO Auto-generated method stub
		DayClosureReportDao dao = new DayClosureReportDao();
		List list = dao.getTodayUpiAmountDao(userType, userName);
		System.out.println("today Upi list size ====> "+list.size());
		Map map = new HashMap();
		for (int i = 0; i < list.size(); i++)
		{
			Object[] o = (Object[]) list.get(i);
